...
def tennisSupportDB () :
    ok = ui.choicesBox("sports-smart-analysis.png" ,"Warrning : Tennis Doesnt Support Database features yet ! are u sure You want to continue ?",["Yes" , "Cancel"])
    if ok =="Yes":
        return 1
    else :
        return 0
...
def sports ():
    sport = ui.choicesBox("sports-smart-analysis.png", "Please Choose the Sport ?",["Football","Tennis" ])
    if sport =="Football":
        return 1
            ...
            while(True):
                ...
                if (player == 1):
                    ...
                    if(hasNumbers(FirstName) or hasNumbers(LastName) or hasNumbers(Nationlity) or hasNumbers(Club) or hasNumbers(Position)):
                        easygui.msgbox("Player name, nationality, club and Position cannot contain Integers","Warnning Message","OK")
                        continue

                    if(not hasNumbers(PlayerNumber) or len(PlayerNumber)>2):
                        easygui.msgbox("Player Number must be atmost two digits and cannot contain char","Warnning Message","OK")
                        continue
